...
function createRoom(ws, msg) {
    ...
    room.chess.white = ws.id;           // host auto WHITE seat
    rooms.set(code, room);
...
function joinRoom(ws, msg) {
    ...
    /* auto seat: pehla = White, doosra = Black */
    if (!room.chess.white) room.chess.white = ws.id;
    else if (!room.chess.black) room.chess.black = ws.id;
    ...
    broadcast(code, { type: 'chess_state', chess: publicChess(room) });
